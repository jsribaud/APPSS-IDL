...
;   GH: 29Nov14      Merged with lbwquicklookwapps
;   KS: Jul15        Integrated lbwremovegps
;   GH: 22Nov18      Fixed error from opening too many files
;                    and running out of logical units
;
PRO lbwquicklook, filename=file, pol=pol, hires=hires, smooth=smooth, xrange=xrange, yrange=yrange, vel=vel, wapps=wapps, board=board, gps=gps, view=view,refresh=refresh
...
if n_elements(file) eq 0 then begin
	spawn, 'ls -t /share/olcor/corfile.*.a2899.*', files
	file = files[0]

; Is it a whole filename (passed a long string)?
endif else if size(file, /tname) eq 'STRING' and strlen(file) gt 30 then begin

	; Did the user say that this is a WAPPS file (or a FITS file)? Then use the WAPPS quicklook
	if keyword_set(wapps) or strlowcase(strmid(file, 4, 5, /reverse)) eq '.fits' then begin
		lbwquicklookwapps, file=file, smooth=smooth, xrange=xrange, yrange=yrange, vel=vel, board=board, gps=gps, pol=pol,refresh=refresh
		return
	endif
...
; Is it the file index? (passed a number)?
	...
	spawn, 'ls -t /share/olcor/corfile.*.a2899.'+strtrim(file,2), files
	file = files[0]
...
if n_elements(filename) eq 0 || file ne filename then begin
	...
	ONOFF_OK = corposonoff(in, cordata, t, cals, bonrecs, boffrecs, /han, /scljy)
	if ONOFF_OK ne 1 then begin 
		print, "(ON-OFF)/OFF failed."
		return
	endif

	;Produce onoff with gps removed
